...
function Configure-SSHAgent {
    ...
    try {
        ...
        if (-not $IsWindows -and $PSVersionTable.PSVersion.Major -ge 6) {
            Write-Warning "This SSH configuration is Windows-specific. Skipping on non-Windows platform."
            return
        }
...
"@
        ...
        if (Test-Path $sshAgentPipe) {
            ...
            Write-Warning "1Password SSH Agent pipe not found. Make sure 1Password is installed and SSH Agent is enabled."
...
function Create-ProjectStructure {
    ...
    try {
        ...
        if (Test-Path $absolutePath) {
            Write-Warning "Directory already exists: $absolutePath"
...
"@
        ...
        Write-Error "Failed to create docker-compose.yml: $_"
        throw
    }
}
