<<<<<<< C:/Users/erich/Heady/HeadyAcademy/HeadyConductor.py
# HEADY_BRAND:BEGIN
...
class HeadyConductor:
    ...
    def _execute_tool(self, tool_name: str, context: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        # Check if it's a Python script we can execute
        if tool.file_path.endswith('.py'):
            try:
                cmd = [sys.executable, tool.file_path]
                
                # Pass arguments based on context/request if possible
                # For HuggingFace_Tool, we expect --task and --input
                if context and "request" in context:
                    # Simple heuristic mapping for demonstration
                    req = context["request"].lower()
                    if "sentiment" in req:
                        cmd.extend(["--task", "sentiment-analysis"])
                    elif "summarize" in req:
                        cmd.extend(["--task", "summarization"])
                    else:
                        cmd.extend(["--task", "text-generation"])
                        
                    # Use the full request as input for now
                    cmd.extend(["--input", context["request"]])
        ...
        # Fallback for non-executable or error
        return {
            "success": True,
            "tool": tool.name,
            "file_path": tool.file_path,
            "category": tool.category,
            "executed_at": datetime.now().isoformat(),
            "note": "Simulation or execution failed"
        }
    ...
    def orchestrate(self, request: str, user_config: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        if execution_plan["nodes_to_invoke"]:
            ...
            for node_info in execution_plan["nodes_to_invoke"]:
                ...
                # Pass request context to node
                node_context = {"request": request}
                result = self.invoke_node(node_info["name"], context=node_context)
...
if __name__ == "__main__":
    main()
>>>>>>> C:/Users/erich/.windsurf/worktrees/Heady/Heady-3392f815/HeadyAcademy/HeadyConductor.py
